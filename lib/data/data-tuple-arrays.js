...
import {maxInColumns} from './data-utils'
...
// /* @return {number} - The max value in specific column of data. */
// function findMax(data, columnIndex, oldMax = Number.NEGATIVE_INFINITY) {
//     let itemsByColumn = data.map(i => i[columnIndex])

//     return Math.max(...itemsByColumn, [oldMax]);
// }
...
export function $multiData(data, targetAttr = null) {
    let dataObj = {
        ...
        _arity: null,
        _summedData: null,
        _maxFinderCallback: maxInColumns,
        ...
        */
        ...
        withMaxFinder(callback) {
            this._maxFinderCallback = callback;
        },
        ...
        max() {
            ...
            this._maxData = [];
            const columnCount = this.dataArity();

            // TODO: This could be a performance issue
            // I send a shallow clone of the data in the maxFinderCallback since I
            // won't alway be the one that is creating the callback and I want
            // to limit the likelihood that the data is changed by the caller
            const shallowDataClone = this.data.map(i => i);
            this._maxData = this._maxFinderCallback(shallowDataClone, columnCount);
